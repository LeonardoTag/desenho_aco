...
namespace CapitalAco.DrawingMacro.App.Services
{
    ...
    public class SkiaRenderer : ISkiaRenderer
    {
        ...
        public void RenderizarPeca(
            ...
            bool omitirRotuloComprimento = false)
        {
            ...
            {
                DesenharChapaLisa(canvas, size, polar.SegmentosOriginal[0].Medida, polar.Comprimento, fonteCota, omitirRotuloComprimento);
                DesenharBadge(canvas, badgeText);
                return;
            }
                ...
                && polar.SegmentosOriginal[0].CurvaInfo!.AnguloCurva >= 359.5)
            {
                ...
                if (mostrarMedidas)
                {
                    DesenharCotaTubo(canvas, coordenadasNoCanvas[0], raioCanvasTubo, dx, dy, infoTubo, polar, fonteCota, omitirRotuloComprimento);
                }
                DesenharBadge(canvas, badgeText);
                return;
            }
            ...
            if (mostrarMedidas && compEsboco > 8.0 && !omitirRotuloComprimento)
            {
                ...
                float fonteComp = Math.Max(13f, fonteCota * 1.5f);
                midCompX += enx * fonteComp * 1.8f;
                midCompY += eny * fonteComp * 1.8f;
        ...
        private static void DesenharBadge(SKCanvas canvas, string? texto)
        {
            if (string.IsNullOrEmpty(texto)) return;
            float badgeFonte = 16f;
            using var badgePaint = new SKPaint
            {
                Color = new SKColor(18, 21, 63),
                TextSize = badgeFonte,
                IsAntialias = true,
                TextAlign = SKTextAlign.Left,
                Typeface = FonteNegrito
            };
            float badgeW = badgePaint.MeasureText(texto);
            float padB = badgeFonte * 0.22f;
            var badgeRect = new SKRect(4f, 4f, 4f + badgeW + padB * 2f, 4f + badgeFonte + padB * 2f);
            using var bgPaint = new SKPaint { Color = new SKColor(255, 255, 255, 210), Style = SKPaintStyle.Fill, IsAntialias = true };
            canvas.DrawRoundRect(badgeRect, 4f, 4f, bgPaint);
            canvas.DrawText(texto, badgeRect.Left + padB, badgeRect.Bottom - padB, badgePaint);
        }
        ...
        private static void DesenharChapaLisa(SKCanvas canvas, SKSize size, double medida, double comprimento, float fonteCota, bool omitirComprimento = false)
        {
            ...
            if (omitirComprimento)
            {
                canvas.DrawText($"{medida:F0}", cx, cy + fonte * 0.35f, dimPaint);
        ...
        private static void DesenharCotaTubo(SKCanvas canvas, SKPoint frente, float raio, float dx, float dy, Segmento.InformacaoCurva info, InstrucoesPolares polar, float fonteCota, bool omitirComprimento = false)
        {
