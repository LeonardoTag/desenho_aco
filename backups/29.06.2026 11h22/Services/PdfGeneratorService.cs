...
namespace CapitalAco.DrawingMacro.App.Services
{
    ...
    public class PdfGeneratorService : IPdfGeneratorService
    {
        ...
        public string GerarRelatorioPedido(List<PecaPedidoItem> itens, string observacao = "")
        {
            ...
            int quantidadeTotalPecas = dadosPecas.Sum(d => d.Item.QuantidadeTotal);
                                ...
                                .Row(row =>
                                {
                                    ...
                                    row.RelativeItem(7).MinHeight(alturaItem).Padding(3).Svg(size => RenderizarComoSvg(size.Width, size.Height, canvas =>
                                    {
                                        if (polar != null)
                                        {
                                            try
                                            {
                                                string badge = !item.TemMultiplosComprimentos && item.QuantidadeTotal > 1 ? $"{item.QuantidadeTotal}x" : "";
                                                _skiaRenderer.RenderizarPeca(canvas, new SKSize(size.Width, size.Height), polar, dim, true, _geometryService, fonteCota: 7f, fonteAngulo: 6.5f, badgeText: badge, omitirRotuloComprimento: item.TemMultiplosComprimentos);
                                            }
                                    ...
                                    row.RelativeItem(2).Padding(5).Column(c =>
                                    {
                                        ...
                                        {
                                            r.RelativeItem().Text($"Qtde: {item.QuantidadeTotal}").Bold().FontSize((float)config.RelatorioPedidoFonteDestaque);
                                        ...
                                        if (item.TemMultiplosComprimentos)
                                        {
                                            c.Item().Text("Comp.:").Bold().FontSize((float)config.RelatorioPedidoFonteDestaque);
                                            foreach (var v in item.VariantesComprimento!)
                                                c.Item().PaddingLeft(6).Text($"{v.Quantidade}× {v.Comprimento:F0} mm").Bold().FontSize((float)config.RelatorioPedidoFonteTexto);
                                        }
                                        else
                                        {
                                            c.Item().Text($"Comp.: {item.Comprimento:F0} mm").Bold().FontSize((float)config.RelatorioPedidoFonteDestaque);
                                        }
                                        if (dim.HasValue)
